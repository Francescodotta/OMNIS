...
const BatchProteomicsUpload = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    rawFiles.forEach((file) => {
      formData.append("proteomics_files", file)
    })
    ...
    try {
      ...
      const response = await proteomicsApi.post(
        `/api/v1/project/${progressive_id}/proteomics_experiment`,
        formData,
        {
          headers: {
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.writeFile(workbook, "proteomics_metadata_schema.xlsx")
  }
        ...
        <Card elevation={3} sx={{ borderRadius: "12px", overflow: "visible" }}>
          ...
          >
            <Typography variant="h4" fontWeight="bold" sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <ScienceIcon fontSize="large" /> Proteomics Data Upload
            </Typography>
            <Typography variant="subtitle1" sx={{ mt: 1, opacity: 0.9 }}>
              Upload your Proteomics RAW files and metadata for analysis
            </Typography>
          </Box>
...
export default BatchProteomicsUpload
